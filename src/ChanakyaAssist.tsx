...
const CACHE_KEY = "bitbuzz:chanakya:trend-cache:v1";
...
export default function ChanakyaAssist({ article, onChange }: Props) {
  ...
  const invoke = async (body: Record<string, unknown>) => {
    ...
    if (!token) throw new Error("Please sign in to the BitBuzz editorial console again, then retry.");
    const { data, error } = await supabase.functions.invoke("chanakya-assist", { body, headers: { Authorization: `Bearer ${token}` } });
  ...
  const loadTrends = async () => {
    setLoadingTrends(true); setError(""); setNotice("");
    try {
      const data = await invoke({ action: "trending" });
      const next = data.topics || [];
      setTopics(next);
      setBackend(data.aiModel ? `AI: ${data.aiModel}` : "Source fallback");
      if (data.fallback) setNotice(data.notice || "AI ranking is temporarily limited. These are fresh source-backed topics.");
      if (next.length) localStorage.setItem(CACHE_KEY, JSON.stringify({ topics: next, savedAt: Date.now() }));
    } catch (e) {
      try {
        const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || "null");
        if (cached?.topics?.length) {
          setTopics(cached.topics); setBackend("Browser cache");
          setNotice("Live trend service is unavailable. Showing your last successfully loaded trend set.");
        } else setError(e instanceof Error ? e.message : "Could not load trends.");
      } catch { setError(e instanceof Error ? e.message : "Could not load trends."); }
    } finally { setLoadingTrends(false); }
  };

  useEffect(() => {
    if (tab !== "trending" || topics.length) return;
    try {
      const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || "null");
      if (cached?.topics?.length) { setTopics(cached.topics); setBackend("Browser cache"); setNotice("Showing your last trend snapshot while Chanakya refreshes."); }
    } catch {}
    loadTrends();
  }, [tab]);
  ...
  const run = async () => {
    ...
    try {
      const data = await invoke({ action: "generate", target, prompt: prompt.trim() || `Write a BitBuzz article about ${article.title || "this topic"}.`, topic: article.title || prompt, article: { title: article.title || "", standfirst: article.standfirst || "", body: article.body_md || "", seoTitle: article.seo_title || "", seoDescription: article.seo_description || "" } });
      const generated = data.article;
      setSources(data.sources || []); setBackend(data.model ? `Groq: ${data.model}` : "Research only");
      if (!generated) { setNotice(data.message || "Research completed, but writing is temporarily unavailable. Nothing was overwritten."); return; }
      ...
      if (data.degraded) setNotice("Chanakya completed this pass using a fallback path. Human editorial review is still required.");
  ...
  const findImages = async () => {
    ...
    setLoading(true); setError(""); setNotice("");
    try { const data = await invoke({ action: "images", query: q }); setImages(data.images || []); setTab("images"); if (!(data.images || []).length) setNotice("No image results from the available search providers."); }
    catch (e) { setError(e instanceof Error ? e.message : "Image search failed."); }
    finally { setLoading(false); }
  };

  const cachedAge = useMemo(() => {
    try { const c = JSON.parse(localStorage.getItem(CACHE_KEY) || "null"); return c?.savedAt ? Math.max(0, Math.round((Date.now() - c.savedAt) / 60000)) : null; } catch { return null; }
  }, [topics]);
  const useImage = (image: ImageResult) => onChange({ ...article, cover_image_url: image.url, cover_alt: image.title });

  return <aside className="overflow-hidden rounded-[22px] border border-white/10 bg-[#080809]">
    <div className="border-b border-white/10 p-4"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#83adff] text-xs font-black text-black">च</span><div><p className="text-sm font-semibold">Chanakya Assist</p><p className="text-[9px] uppercase tracking-[.15em] text-white/30">BitBuzz editorial copilot</p></div></div><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[8px] font-bold uppercase tracking-[.12em] text-emerald-300">Resilient</span></div>
      <div className="mt-4 flex gap-1 rounded-xl bg-white/[.03] p-1">{([["assist","Assist"],["trending","Trending"],["images","Images"]] as [Tab,string][]).map(([id,label])=><button key={id} type="button" onClick={()=>setTab(id)} className={`flex-1 rounded-lg px-2 py-2 text-[10px] font-bold ${tab===id?"bg-white text-black":"text-white/40 hover:text-white"}`}>{label}</button>)}</div>
    </div>
    <div className="p-4">
      {tab === "assist" && <><label className="block"><span className="mb-2 block text-[9px] font-bold uppercase tracking-[.16em] text-white/30">Put the result into</span><select value={target} onChange={e=>setTarget(e.target.value)} className="w-full rounded-xl border border-white/10 bg-[#101011] px-3 py-2.5 text-xs text-white"><option value="whole">Whole article</option><option value="title">Article title</option><option value="standfirst">Standfirst</option><option value="body">Article body</option><option value="seo-title">SEO title</option><option value="seo-description">SEO description</option></select></label>
