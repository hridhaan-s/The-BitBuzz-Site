...
export default function AdminToolsTabs() {
  useEffect(() => {
    ...
    const setup = () => {
      ...
      const show = (kind: "analytics" | "newsletter" | "superadmin" | "opportunities" | "create-opportunity") => { original.forEach(el => { (el as HTMLElement).style.display = "none"; }); mount.classList.remove("hidden"); root.render(kind === "analytics" ? <AnalyticsPanel /> : kind === "newsletter" ? <NewsletterDesk /> : kind === "superadmin" ? <SuperAdminPanel /> : kind === "create-opportunity" ? <AdminOpportunityCreate onDone={() => show("opportunities")} /> : <AdminOpportunities />); };
      const Tools = () => {
        ...
        const item = (name:string, kind:"analytics"|"newsletter"|"superadmin"|"opportunities"|"create-opportunity", accent=false) => <button type="button" onClick={()=>{setActive(name);show(kind)}} className={`w-full rounded-xl px-3 py-2.5 text-left text-xs ${active===name?(accent?"border border-[#ffc48f]/30 bg-[#ffc48f]/10 text-[#ffc48f]":"bg-white text-black"):(accent?"text-[#ffc48f]/70 hover:bg-[#ffc48f]/5 hover:text-[#ffc48f]":"text-white/45 hover:bg-white/5 hover:text-white")}`}>{name}</button>;
        return <div className="space-y-1"><p className="px-3 pb-2 text-[9px] font-bold uppercase tracking-[.2em] text-white/25">Tools</p>{item("Opportunities","opportunities")}{item("Add event","create-opportunity",true)}{item("Newsletter","newsletter")}{item("Analytics","analytics")}{isSuperAdmin&&item("SuperAdmin","superadmin",true)}<button type="button" onClick={()=>{setActive(null);restore()}} className="mt-1 w-full rounded-xl px-3 py-2.5 text-left text-xs text-white/30 hover:bg-white/5 hover:text-white">Back to console</button></div>;
      };
