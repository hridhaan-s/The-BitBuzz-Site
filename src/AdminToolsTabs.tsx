...
export default function AdminToolsTabs() {
  useEffect(() => {
    ...
    const setup = () => {
      ...
      const show = (kind: "analytics" | "newsletter" | "superadmin" | "opportunities") => { original.forEach(el => { (el as HTMLElement).style.display = "none"; }); mount.classList.remove("hidden"); root.render(kind === "analytics" ? <AnalyticsPanel /> : kind === "newsletter" ? <NewsletterDesk /> : kind === "superadmin" ? <SuperAdminPanel /> : <AdminOpportunities />); };
      const Tools = () => {
