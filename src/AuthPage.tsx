...
export default function AuthPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session && mode !== "forgot") window.location.href = "/home";
    });
  }, [mode]);
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      if (!data.session) {
        setMessage("Your account was created. Check your email to confirm your address before signing in.");
        return;
      }
  ...
  const title = mode === "signup" ? "Join BitBuzz." : mode === "forgot" ? "Reset your password." : "Welcome back.";
  const subtitle = mode === "signup" ? "Create your account to write, edit and publish." : mode === "forgot" ? "Enter your email and we'll send you a secure reset link." : "Sign in to write, edit and publish.";
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="flex min-h-[calc(100vh-62px)] items-center justify-center px-5 py-12">
        <div className="w-full max-w-md rounded-[28px] border border-white/10 bg-[#080809] p-7 shadow-2xl sm:p-9">
          ...
          <h1 className="mt-3 font-serif text-5xl tracking-[-.05em]">{title}</h1>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <button type="submit" disabled={busy} className="w-full rounded-xl bg-white py-3.5 text-sm font-bold text-black transition hover:bg-[#83adff] disabled:cursor-wait disabled:opacity-50">
              {busy ? (mode === "signup" ? "Creating account…" : mode === "forgot" ? "Sending reset link…" : "Signing in…") : mode === "signup" ? "Create account" : mode === "forgot" ? "Send reset link" : "Sign in"}
            </button>
          </form>

          {mode === "signin" && (
            <button type="button" onClick={() => switchMode("forgot")} className="mt-5 block w-full text-center text-xs font-semibold text-[#ffe600] underline decoration-[#ffe600]/50 underline-offset-4 transition hover:text-[#fff27a]">
              Forgot password?
            </button>
          )}
          ...
          <div className="mt-7 flex items-center gap-3 text-[12px] text-white/30">
            ...
            <button type="button" onClick={() => switchMode(mode === "forgot" ? "signin" : "signup")} className="font-semibold text-white/50 transition hover:text-white">
              {mode === "forgot" ? "Back to sign in" : "No account? Sign up"}
            </button>
            <span className="h-px flex-1 bg-white/10" />
          </div>
