...
export default function HomeNewsroom({ initialCategory, hideHeader = false }: Props) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-black text-white">
    ...
    <main>
      {/* Compact editorial masthead: the stories start immediately below it. */}
      <section className="border-b border-white/10 bg-black">
        <div className="mx-auto max-w-[1480px] px-5 lg:px-8">
          ...
          <div className="flex flex-col gap-5 py-7 sm:flex-row sm:items-end sm:justify-between lg:py-8">
            <div>
              <p className="text-[9px] font-bold uppercase tracking-[.24em] text-white/40">{copy?.kicker || "BITBUZZ JOURNAL"}</p>
              <h1 className="mt-2 font-serif text-[clamp(2.8rem,5vw,5.5rem)] leading-[.86] tracking-[-.065em]">{copy?.title || "The newsroom"}</h1>
            </div>
            <p className="max-w-md text-[12px] leading-5 text-white/35 sm:text-right">{copy?.deck || "Science, technology, cybersecurity, aviation and innovation — explained clearly, without the noise."}</p>
          </div>
      ...
      <div className="mx-auto max-w-[1480px] px-5 lg:px-8">
        {loading ? <div className="py-24 text-center text-[10px] uppercase tracking-[.2em] text-white/25">Loading the newsroom…</div> : error ? <div className="py-24 text-center text-sm text-red-300/70">Could not load stories.</div> : !lead ? <div className="py-24 text-center"><p className="font-serif text-4xl">No stories yet.</p><p className="mt-3 text-sm text-white/30">Published stories will appear here.</p></div> : <>
          {/* Lead + editorial rail: dense, content-first layout inspired by major financial/newsroom sites. */}
