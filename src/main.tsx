...
function AdminGreeting() {
  useEffect(() => {
    ...
    const setup = async () => {
      const { data: { session } } = await supabase.auth.getSession(); if (!active || !session?.user?.id) return;
      const { data: profile } = await supabase.from("profiles").select("display_name").eq("id", session.user.id).single(); if (!active) return;
...
else if (path === "/admin") page = <><Admin /><AdminGreeting /><AdminMarkdownMount /></>;
