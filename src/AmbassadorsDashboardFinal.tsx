...
type Publication={id:string;slug:string;profile_name:string;school_name:string;description:string|null;logo_url:string|null;organization_type:string;location:string|null;tags:string[];website_url:string|null;instagram_url:string|null;linkedin_url:string|null;contact_email:string|null};
...
export default function AmbassadorsDashboardFinal(){
 ...
 const load=useCallback(async(id:string)=>{
   const[{data:a,error:ae},{data:s,error:se},{data:m,error:me}]=await Promise.all([
     ...
     supabase.from("bitbuzz_publication_members").select("id,name,email,role,photo_url,is_active").eq("publication_id",id).order("created_at")
   ]);
 ...
 const refreshPublication=useCallback(async(id:string)=>{
   ...
   if(p){setPublication(p);setProfileForm({...p,tags:(p.tags||[]).join(", ")});}
 },[]);
 ...
 useEffect(()=>{
   ...
   const boot=async()=>{
     ...
     if(error){setMessage(error.message);return;}
     const p=(pubs||[])[0] as Publication|undefined;
     if(!p)return;
     await supabase.rpc("bitbuzz_claim_publication_membership",{p_publication_id:p.id});
     if(!live)return;
     setPublication(p);setProfileForm({...p,tags:(p.tags||[]).join(", ")});await load(p.id);
   };
   boot();
 ...
 const review=async(s:Submission,status:"approved"|"rejected")=>{setBusy(true);const{data,error}=await supabase.rpc("bitbuzz_review_publication_submission",{p_submission_id:s.id,p_status:status,p_review_note:null});setBusy(false);if(error){setMessage(error.message);return;}setMessage(status==="approved"?(data?"Submission approved and published.":"Submission approved."):"Submission rejected.");if(publication)load(publication.id)};
 const openMemberEditor=(m?:Member)=>{
   if(m){
     setEditingMember(m);
     setMemberForm({name:m.name,email:m.email||"",role:m.role,photo:m.photo_url||""});
   }else{
     setEditingMember(null);
     setMemberForm({name:"",email:"",role:"Ambassador",photo:""});
   }
   setMemberOpen(true);
 };
 const saveMember=async(e:FormEvent)=>{
   ...
   if(editingMember){
     ...
     const result=await supabase.from("bitbuzz_publication_members").update({
       name,email,role,photo_url:photo||null,is_active:true,
     ...
     const result=await supabase.from("bitbuzz_publication_members").insert({
       publication_id:publication.id,user_id:null,name,email,role,photo_url:photo||null,is_active:true
     });
     error=result.error;
   }
   setBusy(false);
