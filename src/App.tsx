...
function BreakingTicker() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (active && Array.isArray(data.headlines)) setHeadlines(data.headlines);
      } catch {
  ...
  return (
    <section className="border-b border-white/10 bg-[#111112] text-white" aria-label="Latest news">
      <div className="mx-auto flex h-11 max-w-[1480px] items-stretch overflow-hidden px-0 sm:px-2">
        <div className="relative z-10 flex shrink-0 items-center gap-2 bg-[#111112] px-4 text-[10px] font-bold tracking-[0.14em] sm:px-5">
          <span className="h-1.5 w-1.5 rounded-full bg-[#35c759] shadow-[0_0_0_4px_rgba(53,199,89,0.1)]" />
          <span className="text-white/80">BREAKING</span>
        </div>
        <div className="relative min-w-0 flex-1 overflow-hidden">
          <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-8 bg-gradient-to-r from-[#111112] to-transparent" />
          <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-[#111112] to-transparent" />
          <div
            className="flex h-full w-max items-center"
            style={{ animation: paused ? "none" : "bitbuzz-ticker 70s linear infinite" }}
          ...
          >
            {items.map((item, index) => (
              <a key={`${item.url}-${index}`} href={item.url} target="_blank" rel="noreferrer" className="group flex shrink-0 items-center gap-3 px-5 text-[12px] text-white/65 transition hover:text-white">
                <span className="max-w-[420px] truncate font-medium">{item.title}</span>
                <span className="text-[9px] uppercase tracking-[0.1em] text-honey/80">{item.source.replace(/^www\./, "")}</span>
